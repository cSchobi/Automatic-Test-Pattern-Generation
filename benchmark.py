...
def run_test(c : Circuit, fault, signalName, inputIndex = None, useOutNode = None):
        c_cpy = copy.deepcopy(c)
        atpg = ATPG(c_cpy, fault, signalName, inputIndex, useOutNode)
        atpg.solve()
        log_info = signalName +\
...
for filename in os.listdir(DIRECTORY):
    ...
    for fault in [Circuit.STUCK_AT_0_FAULT, Circuit.STUCK_AT_1_FAULT]:
        ...
        for inputNodeName in c.inNodes:
            run_test(c, fault, inputNodeName)

        for gateName in c.gates:
            # output of gate
            run_test(c, fault, gateName)
            for i in range(len(c.gates[gateName].inputs)):
                # inputs of gate
                run_test(c, fault, gateName, inputIndex = i)

        for outNodeName in c.outNodes:
            run_test(c, fault, outNodeName, useOutNode = True)
